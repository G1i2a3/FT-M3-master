...
server.put('/posts', (req, res) => {
  ...
  if (!post){
    return res.status(STATUS_USER_ERROR).json({error: "No hay post con ede id"})
  }
...
server.delete('/posts', (req, res) => {
  const { id } = req.body;
  if (!id){
    return res.status(STATUS_USER_ERROR).json({error: "Te olvidsate el id"})
  }  
...
server.delete('/author', (req, res) => {
  ...
  if (!author){
    return res.status(STATUS_USER_ERROR).json({error: "Te olvidsate el autor"})
  }  
